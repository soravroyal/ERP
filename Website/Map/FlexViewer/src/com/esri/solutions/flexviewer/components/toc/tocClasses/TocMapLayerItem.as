...
package com.esri.solutions.flexviewer.components.toc.tocClasses
{
	...
	public class TocMapLayerItem extends TocItem
	{
		...
		private function createChildren():void
		{
			...
			if (layerInfos) {
				...
				for each (var layerInfo:LayerInfo in rootLayers) {
					addChild( createTocLayer(this, layerInfo, layerInfos) );
				}
			}
		}
		...
		private static function createTocLayer( parentItem:TocItem, layerInfo:LayerInfo, layerInfos:Array ):TocLayerInfoItem
		{
			...
			*/
			...
			if (layerInfo.subLayerIds) {
				for each (var childId:Number in layerInfo.subLayerIds) {
					...
					if (childLayer) {
						item.addChild( createTocLayer(item, childLayer, layerInfos) );
					}
				}
			}
