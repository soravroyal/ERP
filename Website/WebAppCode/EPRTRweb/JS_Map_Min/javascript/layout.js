    ...
    mapDeferred.then(function (response) {
        ...
        switch (map.id) {
            ...
            case "map_extendedmap":
                map2 = response.map;
                // add year selector
                addYearSelector(map2, options.years);                
                // add legend
                addLegend(response);
                // add basemap gallery
...
;
			...
			case "map_viewermap":
                map4 = response.map;
                // add year selector
                addYearSelector(map4, options.years);                
                // add legend
                addLegend(response);
                // add basemap gallery
...
function addYearSelector(map, years){
        
        var arrayYears = years.split(",");
        var objYears = new Array();
        var oYear = new Object();
        for (var i = 0; i < arrayYears.length; i++){
            oYear = new Object();
            oYear.label = arrayYears[i];
            oYear.value = "ReportingYear=" + arrayYears[i];
            objYears.push(oYear);
        }

        oYear = new Object();
        oYear.label = "All years";
        if(objYears.length > 2)
                oYear.value = "ReportingYear > 2006";
            else
                oYear.value = "ReportingYear < 2007";
        oYear.selected = true;
        objYears.push(oYear);
...
function showLeftPanel() {	
        ...
        dojo.style(dojo.byId(options.mapName + "legendContainer"), "display", "block");
        ...
        if (document.getElementById("imgButtonCloseFile") != null){
            dojo.byId("imgButtonCloseFile").style.left = "735px";
        }
        }  
...
function hideLeftPanel() {	
    ...
    dojo.style(dojo.byId(options.mapName + "legendContainer"), "display", "none");
	switch (options.mapName) {
            case "map_extended":         
    ...
    if (document.getElementById("imgButtonCloseFile") != null){
        dojo.byId("imgButtonCloseFile").style.left = "940px";
    }
}
			...
			menu.addChild(new dijit.CheckedMenuItem({
				...
				onChange : function() {
					if (layer.layer.featureCollection) {
                        ...
                        else if(!layer.layer.visible && (dojo.byId(options.mapName + "legendContainer").style.display == "block"))                               
							hideLeftPanel();
					}
...
function executeTextSearch(layerID, servicePRTR,text)
 {
	...
	query = new esri.tasks.Query();	
	query.outFields =['*'];
	query.where ="FacilityName LIKE   '%" + text + "%'  OR FacilityID LIKE '%" + text + "%'  OR City LIKE '%" + text + "%'  OR PostalCode LIKE '%" + text + "%'";	
	query.returnGeometry = true;
	// execute and result function
	queryTask.execute(query, function(results) {	
	if (results.features.length==0){
...
function filterDiffuse( layerID, servicePRTR, defExp)
{
if(map1!=undefined)
    {
      var layers = defExp.split(":::")[1].split("::").join(",");

      var mapLayer = map1.getLayer(layerID);
      var layerDef = mapLayer.layerDefinitions;
      var visibleLayers = new Array();
      visibleLayers[0] = layers;
      mapLayer.setVisibleLayers(visibleLayers);
      // show layer with the new filter expression
      mapLayer.show(); 
      mapLayer.resume();
      filterDiffuseExtended(layerID, servicePRTR, defExp);
     }  
}


function filterDiffuseExtended( layerID, servicePRTR, defExp)
{
if(map2!=undefined)
    {
      var layers = defExp.split(":::")[1].split("::").join(",");
