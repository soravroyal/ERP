...
( function() {
        var link = dojo.create("link", {
            type : "text/css",
            rel : "stylesheet",
            href : "javascript/agsjs/dijit/css/TOC.css"
...
dojo.ready(function() {
    ...
    dojo.declare("agsjs.dijit._TOCNode", [dijit._Widget, dijit._Templated], {
        ...
        templateString : '<div class="agsjsTOCNode level${level}">' + '<div data-dojo-attach-point="rowNode" data-dojo-attach-event="onclick:_onClick">' + '<span data-dojo-attach-point="contentNode" class="agsjsTOCContent">' + '<span data-dojo-attach-point="checkContainerNode"></span>' + //<input type="checkbox" data-dojo-attach-point="checkNode"/>' +
        ...
        postCreate : function() {
            ...
            if (this.data._subLayerInfos) {
                ...
                dojo.every(this.data._subLayerInfos, dojo.hitch(this, function(info) {
                    if (info.visible) {
                        noneVisible = false;
                        return false;
                    }
            ...
            if (this.data.collapsed)
                show = false;
            if (this.level >= this.rootLayerTOC.toc.maxLevel) {
                show = false;
            }
            if ((this.level < this.rootLayerTOC.toc.maxLevel) && this.iconNode && this.iconNode.src == this.blank) {
        ...
        _createRootLayerNode : function(rootLayer) {
            console.log("_createRootLayerNode" + rootLayer.visibleLayers.join(","))
        ...
        _createLayerNode : function(layer) {
            ...
            if (layer._subLayerInfos) {// group layer
               ...
               /*** ADDED BY EEA ***/
              //display group layer as visible if any of the sublayers are visible:
                dojo.forEach(layer._subLayerInfos, function(info) {
                    if (info.visible)
                ...
                /*
                 this.rootLayer.visibleLayers = this.rootLayerTOC.info.visibleLayers;
                 */
        ...
        _createChildrenNodes : function(chdn, type) {
            ...
            for (var i = 0, n = chdn.length; i < n; i++) {
                ...
                var params = {
                    ...
                    layer : this.layer,
                    legend : this.legend,
                    level : this.rootLayerTOC._currentIndent
                };
                params[type] = chd;
                params.data = chd;
        ...
        _toggleContainer : function(on) {
            ...
            if (this.iconNode && (dojo.hasClass(this.iconNode, 'dijitTreeExpandoClosed') || dojo.hasClass(this.iconNode, 'dijitTreeExpandoOpened'))) {
        ...
        _onClick : function(evt) {
                ...
                if (this.legend) {
                    ...
                    if ( renderer instanceof esri.renderer.UniqueValueRenderer) {
                        ...
                        if (def.length == renderer.infos.length) {
                            ...
                            this.layer._definitionExpression = renderer.attributeField + ' IN (' + def.join(',') + ')';
                            // def.join(' OR ');
                        }
                    }
            ...
            /**ADDED BY EEA : **/
            /*this.rootLayerTOC.toc.legend.refresh();

            dojo.forEach(this.rootLayerTOC.rootLayer.layerInfos, function(layerInfo) {
            layerInfo.defaultVisibility = dojo.some(this.rootLayerTOC.rootLayer.visibleLayers, function(item) {
            return item === layerInfo.id;
            })
            }, this);*/
    ...
    dojo.declare('agsjs.dijit._RootLayerTOC', [dijit._Widget], {
        ...
        _getLayerInfos : function() {
            ...
            if ((this.rootLayer instanceof (esri.layers.ArcGISDynamicMapServiceLayer) || this.rootLayer instanceof (esri.layers.ArcGISTiledMapServiceLayer))) {
                            ...
                            if (layerInfo.renderer) {
                                if (layerInfo.renderer.infos) {
                                    ...
                                    if (layerInfo.renderer.defaultSymbol) {
                                        dojo.mixin(layerInfo.renderer, legInfo.legend[0]);
                                        //.defaultSymbol
                                    }
    ...
    dojo.declare("agsjs.dijit.TOC", [dijit._Widget], {
         ...
         * @property {Number} [maxLevel] (Added by EEA) Integer max level to drill down toc.
         */
        /**
