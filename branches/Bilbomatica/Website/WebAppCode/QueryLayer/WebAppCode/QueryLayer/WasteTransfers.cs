...
namespace QueryLayer
{
    ...
    public static class WasteTransfers
    {
        ...
        public static MapFilter GetMapJavascriptFilter(WasteTransferSearchFilter filter)
        {
            MapFilter mapFilter = new MapFilter();
            ParameterExpression paramMap = Expression.Parameter(typeof(WASTETRANSFER), "s");
            Expression expMap = LinqExpressionBuilder.GetLinqExpressionWasteTransferSearch(filter, paramMap, false);
            mapFilter.SqlWhere = LinqExpressionBuilder.GetSQLJavascript(expMap, paramMap);
            mapFilter.SqlWhere = "FacilityReportID IN (select FacilityReportID from dbo.WASTETRANSFER where " + mapFilter.SqlWhere + ")";
            return mapFilter;
        }
