...
namespace QueryLayer
{
    ...
    public static class ListOfValues
    {
        ...
        //private static IEnumerable<string> lovPollutantCodeEper = null;
        ...
        public static IEnumerable<LOV_POLLUTANT> PollutantsEPER(int? parentId)
        {
            ...
            IEnumerable<LOV_POLLUTANT> pollutants = getLovPollutantCache().Where(m => object.Equals(m.ParentID, parentId) && m.StartYear == 2001).OrderBy(p => p.Code);
        ...
        public static IEnumerable<LOV_POLLUTANT> GetLeafPollutants(int parentId)
        {
            ...
            foreach (LOV_POLLUTANT p in pollutants)
            {
                ...
                children = children.Where(m => (m.EndYear == null || m.EndYear >= 2007));

                if (children.Count() > 0)
                {
                    leafPollutants.AddRange(children);
                }
                else if(p.EndYear == null || p.EndYear >=2007) //only add EPRTR pollutants
                {
        ...
        {
            //para los combos de las facilities
            if (includeEPER)
            {
        ...
        public static IEnumerable<int> ReportYears2(bool includeEPER)
        {
            //para los combos de las facilities
            if (includeEPER)
            {
        ...
        /* public static IEnumerable<int> ReportYearsEPER(bool includeEPER)
         {
             IEnumerable<int> years = new List<int> { 2001, 2004 }; 
             return years;
         }*/
        ...
        public static IEnumerable<REPORTINGYEAR> ReportingYears(bool includeEPER)
        {
            ...
            {
                int startYear = includeEPER ? 2001 : 2007; //(time Series)
                //int startYear = 2007;
