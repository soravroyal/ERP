...
/// </summary>
    public static class TimeSeriesUtils
    {
        ...
        public static IEnumerable<TimeSeriesUtils.BarDataEPER> InsertMissingYearsEPER(IEnumerable<TimeSeriesUtils.BarDataEPER> data, bool includeEPER)
        {
            IEnumerable<REPORTINGYEAR> years = ListOfValues.ReportingYears(includeEPER);

            IEnumerable<TimeSeriesUtils.BarDataEPER> barsEPER = from y in years
                                                        join c in data on y.Year equals c.Year into g
                                                        from c in g.DefaultIfEmpty()
                                                        orderby y.Year
                                                        select new TimeSeriesUtils.BarDataEPER
                                                        {
                                                            Year = y.Year,
                                                            Values = c != null ? c.Values : null,
                                                            ToolTip = c != null ? c.ToolTip : null
