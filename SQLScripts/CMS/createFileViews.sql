...
-------------------------------------------------------------------------------------
--	This script generates the views for all respective cultures in order to retrive 
--	data from the CMS database via execution of SSIS packages.
--	The information is to be used by the map module.
-------------------------------------------------------------------------------------
...
if object_id('dbo.vAT_en_GB_ChartLabels')is not null DROP view dbo.vAT_en_GB_ChartLabels
go
create view dbo.vAT_en_GB_ChartLabels
as
	select
	cast (k.ResourceKey + '=' + v.ResourceValue as nvarchar(max)) as line
	from tAT_ResourceKey k
	...
	on v.ResourceKeyID = k.ResourceKeyID
	where v.CultureCode = 'en-GB'
	and k.ResourceType like 'ChartLabels'
GO
...
if object_id('dbo.vAT_da_DK_ChartLabels')is not null DROP view dbo.vAT_da_DK_ChartLabels
go
create view dbo.vAT_da_DK_ChartLabels
as
	select
	cast (k.ResourceKey + '=' + v.ResourceValue as nvarchar(max)) as line
	from tAT_ResourceKey k
...
if object_id('dbo.vAT_fr_FR_ChartLabels')is not null DROP view dbo.vAT_fr_FR_ChartLabels
go
create view dbo.vAT_fr_FR_ChartLabels
as
	select
	cast (k.ResourceKey + '=' + v.ResourceValue as nvarchar(max)) as line
	from tAT_ResourceKey k
