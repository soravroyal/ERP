...
/*
  ...
  */  
	...
	order by prevreportingyear,prevNationalID;
	OPEN fac_Cursor;
	FETCH NEXT FROM fac_Cursor into @txt1,@txt2;
	WHILE @@FETCH_STATUS = 0
	BEGIN
	print('Previous NationalID: ' + @txt1 + ' from previous Reportingyear: ' + @txt2 + ' are referenced by the following facilities in the new report')
	DECLARE 
	prev_Cursor CURSOR FOR
	SELECT NationalID
	FROM #HAS_PREVIOUS_ID 
	where prevnationalid = @txt1 and prevreportingyear = @txt2
	order by NationalID;
	OPEN prev_Cursor;
	FETCH NEXT FROM prev_Cursor into @txt3;
	WHILE @@FETCH_STATUS = 0
	begin
	print('NationalID: ' + @txt3 + '')
	FETCH NEXT FROM prev_Cursor into @txt3;
	end;
	CLOSE prev_Cursor;
