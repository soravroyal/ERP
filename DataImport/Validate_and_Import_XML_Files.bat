...
for /f "eol=# tokens=1,2,3,4,5,6,7 delims=;" %%a in (%config%) do (
	...
	if not exist %data_dir%\logs\!filename!.log (
	...
	if %singleMode%==true (
		set /p quit=Press enter to continue with next or Q to quit: 
		if /I !quit! == Q goto END
	)
...
set /p name=All files imported.Press enter
:END
echo Importing done