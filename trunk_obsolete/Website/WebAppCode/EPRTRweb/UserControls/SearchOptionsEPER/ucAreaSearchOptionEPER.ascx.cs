...
public partial class ucAreaSearchOptionEPER : System.Web.UI.UserControl
{
    ...
    private void populateAreaCountry()
    {
        ...
        IEnumerable<LOV_AREAGROUP> groups = QueryLayer.ListOfValues.AreaGroups().Where(x => x.LOV_AreaGroupID != 4);
        IEnumerable<REPORTINGCOUNTRY> countries = QueryLayer.ListOfValues.ReportingCountries().Where(x => x.LOV_CountryID != 127 && x.LOV_CountryID != 182 && x.LOV_CountryID != 214 && x.LOV_CountryID != 101 && x.LOV_CountryID != 34);

        //Value of areas are negative to separate them from countries
        foreach (LOV_AREAGROUP g in groups)
            this.cbFacilityCountry.Items.Add(new ListItem(LOVResources.AreaGroupName(g.Code).Replace("E-PRTR", "EPER"), (-g.LOV_AreaGroupID).ToString()));
