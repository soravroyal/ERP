<%@ Control Language="C#" AutoEventWireup="true" EnableViewState="true" CodeFile="ucWasteTransferActivities.ascx.cs" Inherits="ucWasteTransferActivities" %>
...
<div>
<asp:ListView ID="lvWasteTransferActivity" runat="server" OnItemCommand="OnItemCommand" OnDataBinding="OnDataBinding" OnItemDataBound="rows_OnItemDataBound">
       ...
       <table>
         <thead>
          <tr class="generalListStyle_headerRow">
            ...
            <th scope="col"><div class="WTcolName"><asp:Label ID="lbTransfers" Text="<%$ Resources:WasteTransfers,TransferPerIndustrialActivity %>" runat="server"></asp:Label></div></th>
            <th scope="col"><div class="ColLink"></div></th>
      ...
      <ItemTemplate>
        ...
        <tr id="row" class='<%#GetRowCss(Container.DataItem)%>'>
          ...
          <td title='<%# GetToolTipFacilitySearch()%>' >
            ...
            <asp:ImageButton ID="newSearchFacility" ImageUrl="~/images/newsearch.gif" OnCommand="onNewSearchClick" CommandName='searchFacility' CommandArgument='<%# GetActivityCommandArg(Container.DataItem)%>' runat="server" /> 
            </div>
          </td>
            
          <td>
              <div class="WTcolTotal">
                <div id="Div16" Visible="<%# ShowTreatmentTotal %>" runat="server"><asp:Literal  ID="litTotal" runat="server" Text="<%$ Resources:Common,Total %>" /><br /></div> 
                <div id="Div17" Visible="<%# ShowRecovery %>" runat="server"><asp:Literal ID="litRecovery" runat="server" Text="<%$ Resources:Common,Recovery %>" /><br /></div> 
                <div id="Div18" Visible="<%# ShowDisposal %>" runat="server"><asp:Literal ID="litDisposal" runat="server" Text="<%$ Resources:Common,Disposal %>" /><br /></div> 
                <div id="Div19" Visible="<%# ShowUnspecified %>" runat="server"><asp:Literal ID="litUnspec" runat="server" Text="<%$ Resources:Common,Unspec %>" /></div> 
              </div>
          </td>
          
         <td title='<%# GetHWICToolTip(Container.DataItem)%>'>
            <div class="WTcolHWIC" visible="<%# ShowHWIC%>" runat="server">
              <div id="Div0" runat="server" Visible="<%# ShowTreatmentTotal %>"><%# GetHWICTotal(Container.DataItem)%><br /></div>
              <div id="Div1" runat="server" Visible="<%# ShowRecovery %>"><%# GetHWICRecovery(Container.DataItem)%><br /></div>
              <div id="Div2" runat="server" Visible="<%# ShowDisposal %>"><%# GetHWICDisposal(Container.DataItem)%><br /></div>
              <div id="Div3" runat="server" Visible="<%# ShowUnspecified %>"><%# GetHWICUnspec(Container.DataItem)%></div>
            </div>
          </td>
         ...
         <td title='<%# GetHWOCToolTip(Container.DataItem)%>'>
            <div class="WTcolHWOC" visible="<%# ShowHWOC%>" runat="server">
              <div id="Div4" runat="server" Visible="<%# ShowTreatmentTotal %>"><%# GetHWOCTotal(Container.DataItem)%><br /></div>
              <div id="Div5" runat="server" Visible="<%# ShowRecovery %>"><%# GetHWOCRecovery(Container.DataItem)%><br /></div>
              <div id="Div6" runat="server" Visible="<%# ShowDisposal %>"><%# GetHWOCDisposal(Container.DataItem)%><br /></div>
              <div id="Div7" runat="server" Visible="<%# ShowUnspecified %>"><%# GetHWOCUnspec(Container.DataItem)%></div>
            </div>
          </td>

         <td title='<%# GetHWToolTip(Container.DataItem)%>'>
            <div class="WTcolHWTotal" visible="<%# ShowHWTotal%>" runat="server">
              <div id="Div8" runat="server" Visible="<%# ShowTreatmentTotal %>"><%# GetHWTotal(Container.DataItem)%><br /></div>
              <div id="Div9" runat="server" Visible="<%# ShowRecovery %>"><%# GetHWRecovery(Container.DataItem)%><br /></div>
              <div id="Div10" runat="server" Visible="<%# ShowDisposal %>"><%# GetHWDisposal(Container.DataItem)%><br /></div>
              <div id="Div11" runat="server" Visible="<%# ShowUnspecified %>"><%# GetHWUnspec(Container.DataItem)%></div>
            </div>
          </td>

         <td title='<%# GetNONHWToolTip(Container.DataItem)%>'>
            <div class="WTcolNONHW" visible="<%# ShowNonHW%>" runat="server">
              <div id="Div12" runat="server" Visible="<%# ShowTreatmentTotal %>"><%# GetNONHWTotal(Container.DataItem)%><br /></div>
              <div id="Div13" runat="server" Visible="<%# ShowRecovery %>"><%# GetNONHWRecovery(Container.DataItem)%><br /></div>
              <div id="Div14" runat="server" Visible="<%# ShowDisposal %>"><%# GetNONHWDisposal(Container.DataItem)%><br /></div>
              <div id="Div15" runat="server" Visible="<%# ShowUnspecified %>"><%# GetNONHWUnspec(Container.DataItem)%></div>
            </div>
          </td>
